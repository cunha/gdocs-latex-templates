...
# Change the line below to contain an export URL. Make sure the sharing
# mode on the Doc is set to "anyone with link can view", otherwise
# Google Docs exports comments, which breaks compilation. Add people
# manually to allow edit access. Note the link should end with
# export?format=txt

DOCS_LINK=https://docs.google.com/document/d/1YJg3v-OKCr4KMZUlCs-z6hMgGwmJURUCiztmFF2Gce4/export?format=txt

# Camera-ready template
# DOCS_LINK=https://docs.google.com/document/d/17Yy8kO-9p3tjWJhKiZjVf-LUykfiuZRcdD40_bnnGMA/export?format=txt

# This line should not change; however, you can customize the
# template.tex for the conference
#
# We use "--tab-stop 3" as Google Docs indents nested lists with three
# spaces (for markup, it's normally 4)
PANDOC_FLAGS=-s -N --template=template.tex -f markdown+yaml_metadata_block+footnotes -t latex --tab-stop 3

# Customize the line below to change the bib file and the csl file
# (either ieee or acm) and to use pandoc-citeproc or biblatex (the
# latter is the default)
# BIBLIO_FLAGS=--bibliography=mybib.bib --csl=acm.csl
BIBLIO_FLAGS=--bibliography=references.bib --natbib
...
clean:
	rm -f $(NAME).aux $(NAME).bbl $(NAME).blg $(NAME).log $(NAME).pdf $(NAME).md $(NAME).mdg $(NAME).mdt $(NAME).md-r $(NAME).out $(NAME).trig $(NAME).run.xml $(NAME)-blx.bib $(NAME).download authors.aux
	# CAUTION -- remove if source is moved from Google Docs:
	rm -f $(NAME).tex
...
$(NAME).tex: $(NAME).trig
	...
	sed -i -e '/^\\\iffalse/{x;p;x;G;}' $(NAME).md
	sed -i -e '/^\\\fi/{x;p;x;G;}' $(NAME).md
	rm $(NAME).mdt
